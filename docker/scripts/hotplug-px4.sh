...
function usage() {
	echo "Usage: hotplug-px4.sh -a <action=add/remove> -d <dev_path> [options]"
...
ARGUMENTS=$(getopt -n hotplug-px4.sh -o a:d:M:m:p:h -l action:,dev-path:,major-version:,minor-version:,parent-path:,help -- "$@" )
